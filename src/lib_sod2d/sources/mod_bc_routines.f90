   ...
   implicit none
      ...
      contains
         ...
         subroutine temporary_bc_routine_dirichlet_prim(npoin,nboun,bou_codes,bou_codes_nodes,bound,nbnodes,lbnodes,lnbn,lnbn_nodes,normalsAtNodes,aux_rho,aux_q,aux_u,aux_p,aux_E,u_buffer)
            ...
            real(rp)                   :: aux_rho2(npoin),aux_q2(npoin,ndime),aux_u2(npoin,ndime),aux_p2(npoin),aux_E2(npoin),u_buffer(npoin,ndime)
            integer(4)                 :: iboun,bcode,ipbou,inode,idime,iBoundNode
            real(rp)                   :: cin,R_plus,R_minus,v_b,c_b,s_b,rho_b,p_b,rl,rr, sl, sr
            real(rp)                   :: q_hll,rho_hll,E_hll,E_inf,norm,mag_u_buffer,mag_u_2
            ...
            do inode = 1,npoin
               if(bou_codes_nodes(inode) .lt. max_num_bou_codes) then
                  ...
                  if (bcode == bc_type_far_field) then ! inlet just for aligened inlets with x
                     ...
                     mag_u_buffer = sqrt(dot_product(u_buffer(inode,:),u_buffer(inode,:)))
                     mag_u_2      = sqrt(dot_product(aux_u2(inode,:),aux_u2(inode,:)))
                     ...
                     E_inf = (nscbc_rho_inf*0.5_rp*mag_u_buffer**2 + nscbc_p_inf/(nscbc_gamma_inf-1.0_rp))

                     rho_hll = (sr*aux_rho2(inode)-sl*nscbc_rho_inf+nscbc_rho_inf*mag_u_buffer-aux_q2(inode,1))/(sr-sl)
                     ...
                     E_hll   = (sr*aux_E2(inode)-sl*E_inf+mag_u_buffer*(E_inf+nscbc_p_inf)-mag_u_2*(aux_E2(inode)+aux_p2(inode)))/(sr-sl)
                     ...
                     q_hll   = (sr*aux_q2(inode,2)-aux_u2(inode,2)*aux_q2(inode,2))/(sr-sl)
                     ...
                     q_hll   = (sr*aux_q2(inode,3)-aux_u2(inode,3)*aux_q2(inode,3))/(sr-sl)
