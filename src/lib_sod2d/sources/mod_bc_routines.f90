            ...
            do inode = 1,npoin
               if(bou_codes_nodes(inode) .lt. max_num_bou_codes) then
                  ...
                  if (bcode == 1) then ! inlet just for aligened inlets with x
                     ...
                     sl = min(nscbc_u_inf-nscbc_c_inf, aux_u(lnbn_nodes(inode),1) - sqrt(nscbc_gamma_inf*aux_p(lnbn_nodes(inode))/aux_rho(lnbn_nodes(inode))))
                     sr =  max(aux_u(lnbn_nodes(inode),1) + sqrt(nscbc_gamma_inf*aux_p(lnbn_nodes(inode))/aux_rho(lnbn_nodes(inode))), nscbc_u_inf+nscbc_c_inf)

                     E_inf = (nscbc_rho_inf*0.5_rp*nscbc_u_inf**2 + nscbc_p_inf/(nscbc_gamma_inf-1.0_rp))

                     rho_hll = (sr*aux_rho(lnbn_nodes(inode))-sl*nscbc_rho_inf+nscbc_rho_inf*nscbc_u_inf-aux_q(lnbn_nodes(inode),1))/(sr-sl)
                     q_hll   = (sr*aux_q(lnbn_nodes(inode),1)-sl*nscbc_rho_inf*nscbc_u_inf+nscbc_rho_inf*nscbc_u_inf**2-aux_u(lnbn_nodes(inode),1)*aux_q(lnbn_nodes(inode),1))/(sr-sl)
                     E_hll   = (sr*aux_E(lnbn_nodes(inode))-sl*E_inf+nscbc_u_inf*(E_inf+nscbc_p_inf)-aux_u(lnbn_nodes(inode),1)*(aux_E(lnbn_nodes(inode))+aux_p(lnbn_nodes(inode))))/(sr-sl)
                     ...
                     sl = min(-nscbc_c_inf, aux_u(lnbn_nodes(inode),2) - sqrt(nscbc_gamma_inf*aux_p(lnbn_nodes(inode))/aux_rho(lnbn_nodes(inode))))
                     sr =  max(aux_u(lnbn_nodes(inode),2) + sqrt(nscbc_gamma_inf*aux_p(lnbn_nodes(inode))/aux_rho(lnbn_nodes(inode))), nscbc_c_inf)
                     q_hll   = (sr*aux_q(lnbn_nodes(inode),2)-aux_u(lnbn_nodes(inode),2)*aux_q(lnbn_nodes(inode),2))/(sr-sl)

                     aux_q(inode,2) = q_hll

                     sl = min(-nscbc_c_inf, aux_u(lnbn_nodes(inode),3) - sqrt(nscbc_gamma_inf*aux_p(lnbn_nodes(inode))/aux_rho(lnbn_nodes(inode))))
                     sr =  max(aux_u(lnbn_nodes(inode),3) + sqrt(nscbc_gamma_inf*aux_p(lnbn_nodes(inode))/aux_rho(lnbn_nodes(inode))), nscbc_c_inf)
                     q_hll   = (sr*aux_q(lnbn_nodes(inode),3)-aux_u(lnbn_nodes(inode),2)*aux_q(lnbn_nodes(inode),3))/(sr-sl)
                     ...
                     q_hll   = (sr*aux_q(lnbn_nodes(inode),3)-aux_u(lnbn_nodes(inode),3)*aux_q(lnbn_nodes(inode),3))/(sr-sl)
