            ...
            do inode = 1,npoin
               if(bou_codes_nodes(inode) .lt. max_num_bou_codes) then
                  ...
                  if (bcode == bc_type_inlet) then ! inlet just for aligened inlets with x
                   ...
                   q_hll   = (sr*aux_q2(inode,1)-sl*nscbc_rho_inf*nscbc_u_inf+nscbc_rho_inf*nscbc_u_inf**2-aux_u2(inode,1)*aux_q2(inode,1))/(sr-sl)
                   ...
                   sl = min(-nscbc_c_inf, aux_u2(inode,2) - sqrt(nscbc_gamma_inf*aux_p2(inode)/aux_rho2(inode)))
                   sr =  max(aux_u2(inode,2) + sqrt(nscbc_gamma_inf*aux_p2(inode)/aux_rho2(inode)), nscbc_c_inf)
                   q_hll   = (sr*aux_q2(inode,2)-aux_u2(inode,2)*aux_q2(inode,2))/(sr-sl)

                   aux_q(inode,2) = q_hll

                   sl = min(-nscbc_c_inf, aux_u2(inode,3) - sqrt(nscbc_gamma_inf*aux_p2(inode)/aux_rho2(inode)))
                   sr =  max(aux_u2(inode,3) + sqrt(nscbc_gamma_inf*aux_p2(inode)/aux_rho2(inode)), nscbc_c_inf)
                   q_hll   = (sr*aux_q2(inode,3)-aux_u2(inode,2)*aux_q2(inode,3))/(sr-sl)
