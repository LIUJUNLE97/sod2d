         ...
         subroutine imex2_main(igtime,iltime,save_logFile_next,noBoundaries,isWallModelOn,nelem,nboun,npoin,npoin_w,numBoundsWM,point2elem,lnbn_nodes,lelpn,dlxigp_ip,xgp,atoIJK,invAtoIJK,gmshAtoI,gmshAtoJ,gmshAtoK,maskMapped,&
            ...
            if(iltime .eq. 1) then  
               ...
               if(mpi_size.ge.2) then
                  ...
                  call mpi_halo_atomic_update_real_mass_ener_momentum_iSendiRcv(Rmass(:,1),Rener(:,1),Rmom(:,:,1))
                  call nvtxEndRange
               end if               
            ...
            if(present(source_term) .or.flag_bouyancy_effect) then
               ...
               if(mpi_size.ge.2) then
                  call nvtxStartRange("AB2 halo update")
                  call mpi_halo_atomic_update_real_arrays_iSendiRcv(ndime+2,Rsource(:,:))
                  call nvtxEndRange
               end if
            end if
            ...
            if(mpi_size.ge.2) then
               call nvtxStartRange("AB2 halo update")
               call mpi_halo_atomic_update_real_mass_ener_momentum_iSendiRcv(Rmass(:,2),Rener(:,2),Rmom(:,:,2))
               call nvtxEndRange
            end if
