         ...
         subroutine rk_4_ls_main(noBoundaries,isWallModelOn,nelem,nboun,npoin,npoin_w,numBoundsWM,point2elem,lnbn_nodes,dlxigp_ip,xgp,atoIJK,invAtoIJK,gmshAtoI,gmshAtoJ,gmshAtoK,maskMapped,&
            ...
            do istep = 1,flag_rk_ls_stages
               ...
               if(present(source_term)) then
                  ...
                  call mom_source_const_vect(nelem,npoin,connec,Ngp,dNgp,He,gpvol,u(:,:,pos),source_term(:,1:ndime),Rmom,-1.0_rp)
                  call ener_source_const(nelem,npoin,connec,Ngp,dNgp,He,gpvol,source_term(:,ndime+1),Rener,-1.0_rp)
                  call nvtxEndRange
               end if                 
