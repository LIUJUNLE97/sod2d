module mod_time_ops
   ...
   contains
      ...
      subroutine adapt_dt_cfl(nelem,npoin,connec,helem,u,csound,cfl_conv,dt,cfl_diff,mu_fluid,rho)
         ...
         !$acc parallel loop gang num_gangs(nelem) reduction(min:dt_conv,dt_diff,dt) vector_length(vecLength)
         do ielem = 1,nelem
            L3 = 0.0d0
            ...
            if(present(cfl_diff) .and. present(mu_fluid) .and. present(rho)) then
               ...
               do inode = 1,nnode
                  max_MU = max(max_MU,mu_fluid(connec(ielem,inode))/rho(connec(ielem,inode)))
               end do
