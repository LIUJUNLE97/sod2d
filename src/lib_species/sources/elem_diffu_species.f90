module elem_diffu_species
    ...
    subroutine species_diffusion_ijk(nelem,npoin,connec,Ngp,He,gpvol,dlxigp_ip,invAtoIJK,gmshAtoI,gmshAtoJ,gmshAtoK,Cp,Pr,rho,Yk,mu_fluid,mu_e,mu_sgs,Ml,RYk,initialze,fact)
            ...
            real(rp)                 :: divDy
            real(rp)                 :: rhol(nnode),ykl(nnode),mufluidl(nnode)
            ...
            !$acc parallel loop gang  private(ykl,rhol,mufluidl)
            do ielem = 1,nelem
                !$acc loop vector
                do inode = 1,nnode
                ...
                do igaus = 1,ngaus
                    kappa_y =mufluidl(igaus)*Cp/Pr+c_ener*mu_e(ielem,igaus)*Cp/Pr + Cp*rhol(igaus)*mu_sgs(ielem,igaus)/0.9_rp
